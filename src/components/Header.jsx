...
export const Header = () => {
  ...
  const { currentUser: { username, avatar_url }, setCurrentUser } = useContext(UserContext)
    ...
    <header className="h-20 p-3 flex items-center shadow sticky bg-white top-0 left-0 backdrop-filter backdrop-blur-lg bg-opacity-30">
      ...
      <div className="flex ml-auto space-x-6">
        ...
        <div className="flex space-x-2 items-center">
          ...
          <img src={avatar_url} className="rounded-full border-2 border-neutral-500 w-8 h-8"/>
        </div>
      </div>
    </header>
  )
